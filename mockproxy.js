...
getBucket = function(method, path) {
  ...
  console.log("url", "http://localhost:8082/services/postbucket/?path=" + path + "&random=" + Math.random(1,Math.pow(10, 10)));

  if(method==="PUT") {
    request
      .get("http://localhost:8082/services/putbucket/?path=" + path + "&random=" + Math.random(1,Math.pow(10, 10)))
      .end(function(error, res){
        defer.fulfill(res.body);
      });
  } else if(method==="POST") {
    request
      .get("http://localhost:8082/services/postbucket/?path=" + path + "&random=" + Math.random(1,Math.pow(10, 10)))
      .end(function(error, res){
        defer.fulfill(res.body);
      });
  }
